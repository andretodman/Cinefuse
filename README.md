...
**Audio pipeline.** In production, the `audio` MCP calls real providers (HTTP adapter URLs per tool and/or ElevenLabs for dialogue with an upload endpoint for durable URLs). Runtime stub media is disabled unless test fixtures are explicitly enabled (`NODE_ENV=test`, `CINEFUSE_ALLOW_STUB_MEDIA`, or `--test`). When a provider cannot fulfill a feature, the MCP logs a structured skip, returns a non-blocking response (no Spark debit), and the editor continues; jobs expose skip diagnostics for the status panel. See `docs/CINEFUSE-API-CONTRACT.md` and `.env.example`.
